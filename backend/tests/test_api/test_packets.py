...
class TestGetMeetingSummary:
    ...
    async def test_get_summary(self, client, sample_meeting, sample_flags):
        """Get meeting summary with all sections."""
        response = await client.get(f"{PACKETS_URL}/{MEETING_ID}/summary")
    ...
    async def test_summary_nonexistent_meeting(self, client, sample_store):
        """Summary for nonexistent meeting returns 404."""
        response = await client.get(f"{PACKETS_URL}/{FAKE_MEETING}/summary")
        assert response.status_code == 404

    async def test_summary_flag_stats_by_category(self, client, sample_meeting, sample_flags):
