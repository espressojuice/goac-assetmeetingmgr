...
class TestGetMeetingFlags:
    ...
    async def test_get_all_flags(self, client, sample_flags):
    ...
    async def test_filter_by_severity_red(self, client, sample_flags):
        """Filter flags by severity=red."""
        response = await client.get(f"{FLAGS_URL}/{MEETING_ID}?severity=red")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
        assert all(f["severity"] == "red" for f in data)

    async def test_filter_by_severity_yellow(self, client, sample_flags):
        """Filter flags by severity=yellow."""
        response = await client.get(f"{FLAGS_URL}/{MEETING_ID}?severity=yellow")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
        assert all(f["severity"] == "yellow" for f in data)

    async def test_filter_by_category(self, client, sample_flags):
        """Filter flags by category."""
        response = await client.get(f"{FLAGS_URL}/{MEETING_ID}?category=inventory")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
        assert all(f["category"] == "inventory" for f in data)

    async def test_filter_by_status(self, client, sample_flags):
        """Filter flags by status."""
        response = await client.get(f"{FLAGS_URL}/{MEETING_ID}?status=responded")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    async def test_combined_filters(self, client, sample_flags):
        """Filter flags by multiple criteria."""
        response = await client.get(
            f"{FLAGS_URL}/{MEETING_ID}?severity=red&category=inventory"
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    async def test_invalid_severity_filter(self, client, sample_flags):
        """Invalid severity filter returns 422."""
        response = await client.get(f"{FLAGS_URL}/{MEETING_ID}?severity=orange")
        assert response.status_code == 422

    async def test_nonexistent_meeting(self, client, sample_store):
        """Flags for nonexistent meeting returns 404."""
        response = await client.get(f"{FLAGS_URL}/{FAKE_MEETING}")
        assert response.status_code == 404

    async def test_flag_response_shape(self, client, sample_flags):
...
class TestFlagStats:
    ...
    async def test_flag_stats(self, client, sample_flags):
        """Get correct flag statistics."""
        response = await client.get(f"{FLAGS_URL}/{MEETING_ID}/stats")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 4
    ...
    async def test_stats_nonexistent_meeting(self, client, sample_store):
        """Stats for nonexistent meeting returns 404."""
        response = await client.get(f"{FLAGS_URL}/{FAKE_MEETING}/stats")
        assert response.status_code == 404
...
class TestRespondToFlag:
    ...
    async def test_respond_to_flag(self, client, sample_flags):
    ...
    async def test_respond_nonexistent_flag(self, client, sample_store):
    ...
    async def test_respond_invalid_flag_id(self, client):
    ...
    async def test_respond_missing_fields(self, client, sample_flags):
